namespace GetJob.Models.MenuModel;

public static class Logo
{
    public static void ShowLogo()
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine(@"


                                     $$$$$$\             $$\        $$$$$\           $$\       
                                    $$  __$$\            $$ |       \__$$ |          $$ |      
                                    $$ /  \__| $$$$$$\ $$$$$$\         $$ | $$$$$$\  $$$$$$$\  
                                    $$ |$$$$\ $$  __$$\\_$$  _|        $$ |$$  __$$\ $$  __$$\ 
                                    $$ |\_$$ |$$$$$$$$ | $$ |    $$\   $$ |$$ /  $$ |$$ |  $$ |
                                    $$ |  $$ |$$   ____| $$ |$$\ $$ |  $$ |$$ |  $$ |$$ |  $$ |
                                    \$$$$$$  |\$$$$$$$\  \$$$$  |\$$$$$$  |\$$$$$$  |$$$$$$$  |
                                     \______/  \_______|  \____/  \______/  \______/ \_______/ 
                                                           ");
    }
    public static void ShowSignUpLogo()
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine(@"


                                                     ____ ____ ____ ____ ____ ____ 
                                                    ||S |||i |||g |||n |||U |||p ||
                                                    ||__|||__|||__|||__|||__|||__||
                                                    |/__\|/__\|/__\|/__\|/__\|/__\|
");
    }

    public static void ShowSignInLogo()
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine(@"


                                                     ____ ____ ____ ____ ____ ____ 
                                                    ||S |||i |||g |||n |||I |||n ||
                                                    ||__|||__|||__|||__|||__|||__||
                                                    |/__\|/__\|/__\|/__\|/__\|/__\|
");
    }

    public static void ShowVarificationLogo()
    {
        Console.ForegroundColor = ConsoleColor.Green;
        Console.WriteLine(@"
                                                 __   __        _  __ _         _   _          
                                                 \ \ / /_ _ _ _(_)/ _(_)__ __ _| |_(_)___ _ _  
                                                  \ V / _` | '_| |  _| / _/ _` |  _| / _ \ ' \ 
                                                   \_/\__,_|_| |_|_| |_\__\__,_|\__|_\___/_||_|
                                                               / __|___  __| |___              
                                                              | (__/ _ \/ _` / -_)             
                                                               \___\___/\__,_\___|                                                         
");
    }

    public static void ShowResumesLogo()
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine(@"
                                                  ____                                     
                                                 |  _ \ ___  ___ _   _ _ __ ___   ___  ___ 
                                                 | |_) / _ \/ __| | | | '_ ` _ \ / _ \/ __|
                                                 |  _ <  __/\__ \ |_| | | | | | |  __/\__ \
                                                 |_| \_\___||___/\__,_|_| |_| |_|\___||___/
                                                   
");
    }
    public static void ShowVacanciesLogo()
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine(@"
                                                    __     __                         _           
                                                    \ \   / /_ _  ___ __ _ _ __   ___(_) ___  ___ 
                                                     \ \ / / _` |/ __/ _` | '_ \ / __| |/ _ \/ __|
                                                      \ V / (_| | (_| (_| | | | | (__| |  __/\__ \
                                                       \_/ \__,_|\___\__,_|_| |_|\___|_|\___||___/
                                                                                                   
");
    }
    public static void ShowEmployersLogo()
    {
        Console.ForegroundColor = ConsoleColor.Cyan;
        Console.WriteLine(@"
                                              ___            _                     
                                             | __|_ __  _ __| |___ _  _ ___ _ _ ___
                                             | _|| '  \| '_ | / _ | || / -_| '_(_-<
                                             |___|_|_|_| .__|_\___/\_, \___|_| /__/
                                                       |_|         |__/                                                                       
");
    }

    public static void ShowEmployeesLogo()
    {
        Console.ForegroundColor = ConsoleColor.Cyan;
        Console.WriteLine(@"
                                              ___            _                     
                                             | __|_ __  _ __| |___ _  _ ___ ___ ___
                                             | _|| '  \| '_ | / _ | || / -_/ -_(_-<
                                             |___|_|_|_| .__|_\___/\_, \___\___/__/
                                                       |_|         |__/                                                          
");
    }

    public static void ShowNotificationsLogo()
    {
        Console.ForegroundColor = ConsoleColor.Green;
        Console.WriteLine(@"
                                           __      _   _  __ _           _   _                 
                                        /\ \ \___ | |_(_)/ _(_) ___ __ _| |_(_) ___  _ __  ___ 
                                       /  \/ / _ \| __| | |_| |/ __/ _` | __| |/ _ \| '_ \/ __|
                                      / /\  / (_) | |_| |  _| | (_| (_| | |_| | (_) | | | \__ \
                                      \_\ \/ \___/ \__|_|_| |_|\___\__,_|\__|_|\___/|_| |_|___/
                                                                                                          
");
    }

    public static void ShowNewResumeLogo()
    {
        Console.ForegroundColor = ConsoleColor.Blue;
        Console.WriteLine(@"
                                            __                  __                                
                                         /\ \ \_____      __   /__\ ___  ___ _   _ _ __ ___   ___ 
                                        /  \/ / _ \ \ /\ / /  / \/// _ \/ __| | | | '_ ` _ \ / _ \
                                       / /\  /  __/\ V  V /  / _  \  __/\__ \ |_| | | | | | |  __/
                                       \_\ \/ \___| \_/\_/   \/ \_/\___||___/\__,_|_| |_| |_|\___|
                                                                                                                                                                          
");
    }
    public static void ShowNewVacancyLogo()
    {
        Console.ForegroundColor = ConsoleColor.Blue;
        Console.WriteLine(@"
                                           __                                                      
                                        /\ \ \_____      __ /\   /\__ _  ___ __ _ _ __   ___ _   _ 
                                       /  \/ / _ \ \ /\ / / \ \ / / _` |/ __/ _` | '_ \ / __| | | |
                                      / /\  /  __/\ V  V /   \ V / (_| | (_| (_| | | | | (__| |_| |
                                      \_\ \/ \___| \_/\_/     \_/ \__,_|\___\__,_|_| |_|\___|\__, |
                                                                                             |___/ 
                                                                                                                                                                          
");
    }
    public static void ShowProfileLogo()
    {
        Console.ForegroundColor = ConsoleColor.Green;
        Console.WriteLine(@"
                                                     ____ ____ ____ ____ ____ ____ ____ 
                                                    ||P |||r |||o |||f |||i |||l |||e ||
                                                    ||__|||__|||__|||__|||__|||__|||__||
                                                    |/__\|/__\|/__\|/__\|/__\|/__\|/__\|                                                                                                                                                         
");
    }

    public static void ShowAdminLogo()
    {
        Console.ForegroundColor = ConsoleColor.Red;
        Console.WriteLine(@"
                                                           ____ ____ ____ ____ ____ 
                                                          ||A |||D |||M |||I |||N ||
                                                          ||__|||__|||__|||__|||__||
                                                          |/__\|/__\|/__\|/__\|/__\|                                                                                                                                                    
");
    }
}
